...
function appUrl() {
  // OAuth uses PKCE. The callback must return to the same host that started
  // sign-in so the browser sends the PKCE verifier cookie back to the app.
  // Vercel gives previews their own hostname, so use that hostname instead of
  // the production URL when a preview deployment initiates authentication.
